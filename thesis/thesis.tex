...
\chapter{Fundamentals}
    ...
    These transformations can be trained along with the others weights of the network allowing wach neuron to model its own optimal shape.
    ...
    In particular, whenever we add a constant number of weights to a fixed activation function, we deal with 'parametric activation functions'. Some examples of this class of functions are: the \textit{Generalized Hyperbolic Tangent} (Ref. )
    ...
    What happens if we give to activation functions grater modelling capabilities, what if we allow them to model any continuous segment? This question is addressed by the class of non-parametric activation functions. These methods,
    ...
    (KAFs), highlighting their properies and experimental results.
    ...
    \subsection{Adaptive Piece-Wise Linear Activation Functions}
            ...
            \end{equation}
            where $S$ is a user-defined value and $a_i$ are the parameter to learn. APL add $2S$ new parameters per neuron i.e. introducing a linear number of parameters to the overall network, which is often feasable.
            APL cannot, however, model or approximate all piece-wise functions, but only saturating ones. Moreover, APLs introduce $S$ non-differentiable points which may harm backpropagation.
    ...
    a sufficiently very large $S$, any smooth function albeit in practice, due to the global effect that any parameter has on the global shape, such approximation
    ...
    Let $\{ x_1, x_2, \cdots, x_S \}$ be an equally spaced sampling of real values symmetric to the origin and step size $\Delta x$ and call \textit{knots}
    the corresponding y-valules $\{\operatorname{SAF}(x_i)\}^S_0$ . Denote $u=\frac{t}{\Delta x}-\left\lfloor\frac{t}{\Delta x}\right\rfloor$ to be the normalized ascissa 
    value between to consecutive knots when the activation is $t$. Finally we can define a SAF at $t$:
    \begin{equation}
        \label{saf}
        \operatorname{SAF}(t)=\mathbf{u}^{T} \mathbf{B} \mathbf{q}_{k},
    \end{equation}
    where $\mathbf{u}=\left[u^{P}, u^{P-1}, \ldots, u^{1}, 1\right]^{T}$, $P$ user-defined value usually chosen to be 3, $\mathbf{q}_{k}$ the vector composed by
    ...
    Good news iwith SAFs is that each knot has only local effect on the overall shape. Therefore, their training allows for faster and better convergence
    to the optimal with respect to the initialized knots. As for polynomial interpolation, SAFs can in principle approximate any smooth function. A drawback, however, 
    comparing to ALF, is that regularization cannot be explicitly implemented for knots.
