...
Swish smooth activation functions.
    ...
    \section{VGG Inspired Architectures Results}
        ...
        respectively, 0.0001 and 0.08. We adversarially train for 60 epochs. At the end of the training the model achieves 78.1\% of accuracy 
        on the test set. Lastly, PGD is performed and the overall accuracy of our network against the perturbed images is 45.41\%. Therefore, we were able to
        develop a relatively resilient networks against full PGD attacks. To be precise, 
        ...
        \begin{figure}[h!]
            ...
            \label{fig:pgditers}
        \end{figure}
        For this reason we are going to consider only PGD with 50 steps when evaluating.
        ...
        apparently in contrast with the claims in (Ref. SAT), indeed, according the paper, we should have observed an increase in both accuracy and robustness with respect to 
        the baseline. Nevertheless, this drop could also be explained with a wrong choice of learning rates bounds during LRF. To better understand why this is happening, we test other activation functions, this time using KAFs. In particular, KAFs with randomly initialized mixing 
        ...
        It is reasonable to think that the promising gains that we experiment with KAFs might help us in improving or even leveraging the properties of smooth activation functions
        to achieve better scores. Specifically, we build another network where activation functions are KAFs with same dictionary size but, in this case, mixing coefficients
        ...
        results with ELU and KAF\_ELU are caused by the architecture itself, which is somehow less suitable to ELUs like shapes.

        We are then left to test the activation function which gained top results in (Ref. SAT), the Swish function. Accordingly to our expectations, with Swish the 
        , 
        - Relu Case : number of parameters, epochs, time per epoch, result,
        - Kaf Case: number of params, epochs, time per epoch, results,
        - ELU 
        - Swish 
        - Elu Kaf
        - Swish Kaf,
        - Free Kaf 
        - Evolution of the attack perturbation 
