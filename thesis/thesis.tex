...
\chapter{Fundamentals}
    ...
    believing that the correct classification is indeed $ \hat{y} $. We name this two different scenarios \textit{indiscriminate} and \textit{targeted} attacks. Moreover, despite the fact that the attack is targeted 
    ...
    \subsection{Another Optimization problem}
        ...
        \end{equation}
        where $\epsilon$ is the size of the biggest perturbation allowed, i.e. if for example we are dealing with images, any pixel will be $[- \epsilon, \epsilon] $ perturbed. 

        Other norms such as $L_2$ are also very common.  
        
        ENDI
        Such attacks, creating adversarial examples [4], may,
        e.g., cause an autonomous vehicle to fail to recognize a
        road sign, cause an automated system to falsely target a
        civilian vehicle, or grant image or audio authentication
        access to a building, machine, or to restricted information.
        Great interest in AL was inspired by Szegedy et al. [4], who
        showed DNNs are quite susceptible to TTEs. A TTE attack
        may be on physical objects in the   world (e.g., altering
        a road sign, camouflaging a vehicle, or darkening a room
        to make it more difficult to identify objects of interest).
        Alternatively, there may be alterations of data objects that
        have either already been digitally captured (digital images,
        voice files) or those which are natively digital, e.g., emails,
        documents, or computer programs. If a TTE attack is
        enabled by an RE attack, which learns a surrogate classifier
        (rather than assuming the TTE attacker possesses perfect
        knowledge of the actual classifier), an important property
        is transferability of the TTE attack [30]: does a perturba-
        tion of a pattern that induces a targeted or indiscriminate
        change in the surrogate classifier’s decision also induce
        such a change in the actual classifier’s decision?
