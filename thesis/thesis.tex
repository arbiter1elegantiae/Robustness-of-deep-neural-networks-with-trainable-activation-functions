...
Swish smooth activation functions.
    ...
    \section{VGG Inspired Architectures Results}
        ...
        due to its simplicity and great performances was to implement and evaluate a ResNet neural 
        network \ref{dnns}.  
    ...
    \section{Exploding Gradients with KafNets}
        ...
        of the network in three sections articulated by the features map dimensionality which are $\{32, 16, 8\}$ respectively. Instead of using 
        ...
        \begin{equation}
            6n+2
        \end{equation}
        stacked weighted layers, where $n$ denotes the number of residual block in each stage.
        ...
        worse than the originally reported 94\%, is still sufficiently accurate. We then manage to have a working implementation for ResNet and we can start our evaluations.  
        ...
        \begin{figure}[h!]
            ...
            \label{fig:explgrad}
        \end{figure} 
        As can be seen from Fig. \ref{fig:explgrad}, after 10 epochs the classification is still performing randomly. Moreover, this trend does not change even if we let the network train longer or if we tune learning rate, KAF parameters 
        or convolutive's kernel weights initialization.    
