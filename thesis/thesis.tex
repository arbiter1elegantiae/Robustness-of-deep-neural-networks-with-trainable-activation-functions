...
In this chapter, experiments assessing the robustness of Kafnets are discussed.
As seen in the previous chapter, this seems to be a direction worth exploring. In fact, vanilla 
Kafnets already show some sort of resiliency against attacks and, in particular, they are also good candidates in 
improving the effects of adversarial training. Thanks to the smoothness of Gaussian KAFs \ref{SAT}
and to their adaptable nature, we conjecture KAFs will improve both the inner maximization of adversarial
training:
\begin{equation}
    \delta = \delta + \alpha \cdot sign(\nabla_{\delta}\operatorname{LS}(f_\theta(x + \delta), y)),
\end{equation}
where the adversarial attack is crafted, and the outer minimization:
\begin{equation}
    \theta = \theta + \beta \cdot \nabla_{\theta}\operatorname{LS}(f_\theta(x + \delta), y),
\end{equation}
where we try to counter the effects of the attack, by making the net robust to adversarial perturbations.

Adversarial training will be implemented with the enhanced FSGM AT discussed above \ref{fbf}, since it allows
us to perform multiple evaluations in reasonable time. For each experiment we will be comparing the 
accuracy of an adversarially trained Kafnet against the same adversarially trained net, this time 
equipped with other different activation functions, ranging from the ReLU baseline
