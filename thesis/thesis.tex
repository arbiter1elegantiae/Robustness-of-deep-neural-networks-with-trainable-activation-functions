...
\chapter{Fundamentals}
    ...
    \section{Deep Neural Networks}
        ...
        \end{figure}
        To approach the problem we can start with the following consideration: if we assume a shallow
        network for a given task reaches an accuracy $x$, then by adding identity layers on such network i.e. layers implementing the identity function,
         will result in a deeper network with again accuracy $x$, it can't get much worse.
        Building upon this argument, authors in (Ref. ResNet paper) conjectured that a deep network will rather
        learn a better mapping starting from $ \mathcal{F} (x) + x $ than from $ \mathcal{F} (x) $, where $ \mathcal{F} (x) $ 
        is the output of the last layer in the shallow network.
